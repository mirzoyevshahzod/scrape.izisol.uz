...
<head>
    ...
    <title>Qozoq Data Scraper</title>
                            ...
                            <div class="row justify-content-center">
                                <div class="col-md-10">
                                    <div class="text-center mb-4">
                                        ...
                                        <h2 class="page-title" style="font-size: 2.5rem;">Qozoq Data Scraper</h2>
                                        <p class="page-subtitle">Qozoqdan ma'lumotlarni avtomatik yig'ish tizimi</p>
                                    </div>
                                        ...
                                        <div class="card-body p-4">
                                            <div class="d-flex gap-2 mb-3">
                                                <button type="button" class="btn btn-outline-primary" 
                                                    onclick="window.open('https://cgr.qoldau.kz/ru/start', '_blank')">
                                            ...
                                            <script>
                                                function copyLink() {
                                                    navigator.clipboard.writeText('https://cgr.qoldau.kz/ru/start');
                                            ...
                                            <form id="scrapeForm">
                                                <div class="mb-4">
                                                    <label for="checkpoint" class="form-label fw-bold">
                                                        <i class="fas fa-map-marker-alt me-2 text-primary"></i>Chegara punktini tanlang:
                                                    </label>
                                                    <select name="checkpoint" id="checkpoint" class="form-select form-select-lg" required>
                                                        <option value="">-- Chegara punktlari --</option>
                                                        @foreach($checkpoints as $id => $name)
                                                            <option value="{{ $id }}">{{ $name }}</option>
                                                        @endforeach
                                                    </select>
                                                    <div class="form-text">
                                                        <i class="fas fa-info-circle me-1"></i>
                                                        Ma'lumotlarni yig'ish uchun chegara punktini tanlang
                                                    </div>
                                                </div>
...
.addEventListener('submit', async function(e) {
    ...
    const checkpointText =
        ...
        .options[
            document.getElementById('checkpoint').selectedIndex
        ].text;
    ...
    try {
        ...
        const response = await fetch(
            `/api/qozoq/scrape/${encodeURIComponent(checkpoint)}`
        );
        ...
        if (groups.includes(checkpoint)) {
            ...
            fileName =
                slugify(checkpointText);
        }

     status.innerHTML =
    '⏳ Scraping davom etmoqda...';

// FILE YARATILGAN VAQTNI TEKSHIRISH UCHUN
...
const interval = setInterval(async () => {
    ...
    try {
        ...
        const found = filesData.files.find(f => {
        ...
        if (found) {
            ...
            status.innerHTML =
                '✅ Tayyor! Fayl yuklanmoqda...';

            // AUTO DOWNLOAD
            window.location.href =
                `/api/qozoq/download/${encodeURIComponent(found)}`;
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch('/api/qozoq/files');
        const data = await response.json();
...
document.getElementById('belarusSelect').addEventListener('submit', async function(e) {
    e.preventDefault();
    ...
    status.innerHTML = '⏳ Yuklanmoqda...';

    try {
        const response = await fetch(`/api/qozoq/download/${encodeURIComponent(file)}`);
        ...
        status.innerHTML = '✅ Fayl yuklab olindi';
    } catch (error) {
        console.error(error);
